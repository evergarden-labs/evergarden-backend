...
/** `@Valid` 검증과 서비스 위임을 확인한다 — 다른 컨트롤러 테스트와 같은 이유로 {@code @WebMvcTest}를 쓴다. */
@ActiveProfiles("test")
@WebMvcTest(controllers = ReplyReportController.class)
...
class ReplyReportControllerTest {
    ...
    @Test
    @DisplayName("정상 신고는 로그인한 사용자 ID로 reportReply에 위임한다")
    void 정상_신고() throws Exception {
        given(reportService.reportReply(eq(1L), eq(11L), any()))
                .willReturn(new ReportResult(1L, ReportStatus.PENDING));

        mvc.perform(post("/replies/11/reports")
