...
@ActiveProfiles("test")
@WebMvcTest(controllers = CommentReportController.class)
...
class CommentReportControllerTest {
    ...
    @Test
    @DisplayName("없는 댓글을 신고하면 서비스의 404가 그대로 전달된다")
    void 없는_댓글() throws Exception {
        given(reportService.reportComment(eq(1L), eq(99L), any()))
                .willThrow(new BusinessException(ErrorCode.COMMENT_NOT_FOUND));

        mvc.perform(post("/comments/99/reports")
    ...
    void 정상_신고() throws Exception {
        given(reportService.reportComment(eq(1L), eq(10L), any()))
                .willReturn(new ReportResult(1L, ReportStatus.PENDING));

        mvc.perform(post("/comments/10/reports")
                        .header("Authorization", "Bearer " + accessToken)
                        .contentType(MediaType.APPLICATION_JSON)
                        .content("""
                                {"reason":"ABUSE"}
